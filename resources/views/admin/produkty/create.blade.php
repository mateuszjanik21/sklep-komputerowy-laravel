@extends('layouts.admin', ['title' => 'Dodaj nowy produkt'])

@section('content')
<div class="card">
    <div class="card-header">
        <h4>Formularz nowego produktu</h4>
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route('admin.produkty.store') }}">
            @csrf
            <div class="row">
                <div class="col-md-8">
                    <div class="mb-3">
                        <label for="Nazwa" class="form-label">Nazwa produktu</label>
                        <input type="text" class="form-control" id="Nazwa" name="Nazwa" required value="{{ old('Nazwa', '') }}">
                    </div>
                    <div class="mb-3">
                        <label for="Opis" class="form-label">Opis</label>
                        <textarea class="form-control" id="Opis" name="Opis" rows="10">{{ old('Opis', '') }}</textarea>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            <select class="form-select" id="KategoriaId" name="KategoriaId" required>
                                ...
                                @foreach($kategorie as $kategoria)
                                    <option value="{{ $kategoria->Id }}" @if(old('KategoriaId') == $kategoria->Id) selected @endif>
                                        {{ $kategoria->Nazwa }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" step="0.01" class="form-control" id="Cena" name="Cena" required value="{{ old('Cena', '') }}">
                    </div>
                    <div class="mb-3">
                        <label for="IloscNaStanie" class="form-label">Ilość na stanie</label>
                        <input type="number" class="form-control" id="IloscNaStanie" name="IloscNaStanie" required value="{{ old('IloscNaStanie', '') }}">
                    </div>
                    <div class="mb-3">
                        <label for="UrlZdjecia" class="form-label">URL zdjęcia</label>
                        <input type="url" class="form-control" id="UrlZdjecia" name="UrlZdjecia" value="{{ old('UrlZdjecia', '') }}">
                    </div>
                </div>
            </div>
            ...
            <div id="specyfikacje-kontener">

            </div>
            
            <div class="input-group mt-3">
                <select id="select-atrybutu" class="form-select" disabled>
                    <option>Najpierw wybierz kategorię</option>
                </select>
                <button type="button" id="dodaj-specyfikacje-btn" class="btn btn-outline-success" disabled>Dodaj wybraną</button>
            </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        kategoriaSelect.addEventListener('change', function() {
            ...
            index = 0;
            if (kategoriaId) {
                wczytajAtrybutyDlaKategorii(kategoriaId);
            } else {
                atrybutSelect.innerHTML = '<option>Najpierw wybierz kategorię</option>';
                atrybutSelect.disabled = true;
                dodajBtn.disabled = true;
            }
        });
